  ...
  try {
    ...
    for (const categorie of categoriesFormations) {
      ...
      if (existingCategorie) {
        console.log(`Catégorie ${categorie.id} existe déjà avec l'id: ${existingCategorie.id}`);
        categoriesMap.set(categorie.id, existingCategorie.id);
        continue;
      }
      ...
      try {
        ...
        await prisma.programmeFormation.create({
          data: {
            code: code,
            type: "sur-mesure",
            titre: programme.titre || "Programme personnalisé",
        ...
        console.log(`Programme personnalisé importé: ${programme.titre || code} (${code})`);
        totalPersImported++;
      } catch (error) {
